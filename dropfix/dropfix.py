...
def main():
    ...
    print(
        f"Will {CYAN}{'simulate ignoring' if args.dry_run else 'ignore'}{RESET} these directories in {CYAN}{dropbox_path}{RESET}:"
    )
...
def ignore_directory(path, system):
    ...
    try:
        if system == "Windows":
            ...
            subprocess.run(["xattr", "-w", "com.dropbox.ignored", "1", path_str], check=True, capture_output=True)
        else:  # Linux and others
            # Linux: attributes
            subprocess.run(["attr", "-s", "com.dropbox.ignored", "-V", "1", path_str], check=True, capture_output=True)
        return True
